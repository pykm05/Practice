...
{
    // TODO
    // Check if user inputted name is valid
    int index = -1;
    for (int i = 0; i < candidate_count; i++)
    {
        // If user inputted name is valid
...
void sort_pairs(void)
{
    ...
    for (int j = 0; j < pair_count; j++)
    {
        ...
        {
            int temp_strength = preferences[pairs[i].winner][pairs[i].loser] - preferences[pairs[i].loser][pairs[i].winner];
            if (temp_strength > current_strength)
            {
                max_index = i;
                current_strength = preferences[pairs[i].winner][pairs[i].loser] - preferences[pairs[i].loser][pairs[i].winner];
            }
        }
